...
resource "google_cloud_run_service" "air-quality" {
...
resource "google_cloud_run_service_iam_policy" "noauth" {
  ...
  service  = google_cloud_run_service.air-quality[0].name

  policy_data = data.google_iam_policy.noauth[0].policy_data
}

output "cloud_run_instance_url" {
  value = var.first_time ? null : google_cloud_run_service.air-quality[0].status.0.url
}